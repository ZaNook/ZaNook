...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "from_add1")) {
  $insertSQL = sprintf("INSERT INTO tw_symptom (t_name, t_date, t_symptom, t_id) VALUES (%s, %s, %s, %s)",
  ...
  $insertGoTo = "admin_login_success1.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<div data-role="header"><h1><img src="images/q1.png" width="300" height="100">   </h1>
    ...
    <p>ตอนรับผู้ใช้งาน: <?php echo $row_Rs_member['name']; ?> <a href="index.php">ออกจากระบบ</a></p>
   ...
   <form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="from_add1">
     <div data-role="fieldcontain">
       ...
       <input type="text" name="t_id" id="t_id" value=""  />
     </div>
     
     <div data-role="fieldcontain">
       <label for="textinput">ชื่อ:</label>
       <input type="text" name="t_name" id="t_name" value=""  />
     </div>
     
      <div data-role="fieldcontain">
       <label for="date">วันที่:</label>
       <input type="date" name="t_date" id="t_date" value=""  />
     </div>
     
      <div data-role="fieldcontain">
       <label for="textinput">อาการ:</label>
       <input type="text" name="t_symptom" id="t_symptom" value=""  />
     </div>
     
      
      <button data-icon="star">บันทึก</button>
      <input type="hidden" name="MM_insert" value="from_add1" />
